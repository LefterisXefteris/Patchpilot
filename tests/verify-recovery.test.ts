...
const errorDeploy: DeployCheckDependencies = {
  listDeployments: async () => ({ deployments: [{ uid: 'dpl_1', state: 'ERROR', target: 'production' }] }),
};

const resolvedSentry: SentryCheckDependencies = {
  getIssue: async () => ({ status: 'resolved', count: '10', lastSeen: new Date(Date.now() - 30 * 60_000).toISOString() }),
};

const activeSentry: SentryCheckDependencies = {
  getIssue: async () => ({ status: 'unresolved', count: '50', lastSeen: new Date(Date.now() - 60_000).toISOString() }),
};
