...
export async function checkVercelDeployment(
  ...
  try {
    const { deployments } = await fetcher.listDeployments({ token: config.vercel.token, teamId, projectId });
    ...
    const ageSeconds = latest.createdAt ? Math.round((Date.now() - latest.createdAt) / 1000) : undefined;
